...
// Initialize database connection for initial load
$database = new Database();
$db = $database->getConnection();

// Get all documents with joins for initial display
$query = "SELECT 
    d.id AS dokumen_id,
...
ORDER BY b.id, s.id, d.id DESC";

$stmt = $db->prepare($query);
$stmt->execute();
$documents = $stmt->fetchAll();

// Get BAB list for filter
$babQuery = "SELECT id, judul, kode, tahun FROM bab ORDER BY id";
        ...
        <div class="row g-3 align-items-end">
            ...
            <div class="col-md-4">
                ...
                <select id="babFilter" class="form-select">
                    ...
                    <?php foreach ($babList as $bab): ?>
                        <option value="<?= $bab['id'] ?>">
                            <?= htmlspecialchars($bab['judul']) ?> 
                            (Kode: <?= $bab['kode'] ?>)
                        </option>
                    <?php endforeach; ?>
                </select>
        ...
        <div class="document-grid" id="documentGrid">
            <?php foreach ($documents as $doc): ?>
                <div class="document-card" data-id="<?= $doc['dokumen_id'] ?>">
                    ...
                    <h3 class="doc-title"><?= htmlspecialchars($doc['dokumen_judul']) ?></h3>
                    
                    <?php if (!empty($doc['elemen_text'])): ?>
                        <div class="doc-elemen">
                            <i class="bi bi-list-check me-1"></i>
                            <?= htmlspecialchars(substr($doc['elemen_text'], 0, 100)) . (strlen($doc['elemen_text']) > 100 ? '...' : '') ?>
                        </div>
                    <?php endif; ?>
                    
                    <?php if (!empty($doc['standart_nama'])): ?>
                        <div class="doc-standart">
                            <strong><i class="bi bi-tag me-1"></i> <?= htmlspecialchars($doc['standart_nama']) ?></strong>
                            <br>
                            <small><?= htmlspecialchars(substr($doc['standart_judul'] ?? '', 0, 80)) ?></small>
                        </div>
                    <?php endif; ?>
                    
                    <div class="doc-bab">
                        <i class="bi bi-book me-1"></i>
                        <?= htmlspecialchars($doc['bab_judul'] ?? 'N/A') ?>
                        <?php if (!empty($doc['bab_tahun'])): ?>
                            (<?= $doc['bab_tahun'] ?>)
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="doc-meta">
                        ...
                        <span class="doc-id">#<?= $doc['dokumen_id'] ?></span>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
...
$(document).ready(function() {
    ...
    function updateDocuments(data) {
        ...
        if (data.data.length === 0) {
            grid.html(`
                <div class="empty-state">
                    ...
                    ${data.search ? `<p>Maaf, tidak ada dokumen yang cocok dengan kata kunci "<strong>${data.search}</strong>"</p>` : '<p>Belum ada dokumen yang tersimpan</p>'}
                </div>
            `);
            return;
        }
        ...
        data.data.forEach((doc, index) => {
            ...
            const elemen = highlightText(doc.elemen || '', data.search);
            const standartNama = highlightText(doc.standart_nama || '', data.search);
            const standartJudul = highlightText(doc.standart_judul || '', data.search);
            ...
            html += `
                <div class="document-card" data-id="${doc.id}" style="animation-delay: ${(index + 1) * 0.05}s">
                    ...
                    ${standartNama ? `
                        <div class="doc-standart">
                            <strong><i class="bi bi-tag me-1"></i> ${standartNama}</strong>
                            <br>
                            <small>${standartJudul ? (standartJudul.length > 80 ? standartJudul.substring(0, 80) + '...' : standartJudul) : ''}</small>
                        </div>
                    ` : ''}
